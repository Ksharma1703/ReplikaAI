...
def SendMessage(browser, text):
    ...
    try:
        ...
        browser.find_element(By.ID, "upload-image-to-chat").send_keys(text)
...
def main():
    ...
    dataPic = pd.read_csv('pic-data.csv')
    try:
        for i in range(0, 5):
            print("Input to Replika - ")
            print(dataPic.iloc[i]['Requests'])
            print(os.getcwd()+"/pic/"+dataPic.iloc[i]['Requests'])
            # browser.execute_script("window.scrollTo(0,0)")
            print("Send Image: ")
            browser.find_element(By.CSS_SELECTOR, ".UploadImageButton__FileInputLabel-sc-qiwfgp-0").click()
            browser.find_element(By.ID, "upload-image-to-chat").send_keys(os.getcwd()+"/pic/"+dataPic.iloc[i]['Requests'])
            # browser.execute_script("window.scrollTo(0,0)")

            print(os.getcwd()+"/pic/"+dataPic.iloc[i]['Requests'])
            # SendMessage(os.getcwd()+"/pic/"+dataPic.iloc[i]['Requests'])
            ...
            sample_sentences_list = [s.rstrip(' ').lstrip(' ') for s in list(dataPic.iloc[i]['Responses'][1:-1].split(","))]
